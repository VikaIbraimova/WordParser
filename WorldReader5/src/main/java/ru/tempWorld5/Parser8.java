...
public class Parser8 {
    ...
    public void parseOutExcel2(Map<String,String> cellData){
        ...
        int cellNum;
        for (Map.Entry entry : cellData.entrySet()){
            System.out.println("Key(Столбец): " +entry.getKey());
            CellReference cellReference = new CellReference((String) entry.getKey());
            cellNum = cellReference.getCol();
            //Смотрим номера столбцов
            System.out.println("Номера столбцов, куда пишем: "+ cellNum);
            System.out.println("Value(Строка): " +entry.getValue());
            int rownum = cellReference.getRow();
            System.out.println("Номера строк, куда пишем: " + rownum);
            row = outputFileSheet.createRow(rownum);
            //cell = row.createCell(cellNum);
            cell = row.createCell(cellNum,rownum);
            cell.setCellValue("vika");
            //cellNum = 0;
        }
