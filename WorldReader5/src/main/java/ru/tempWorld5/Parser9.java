...
public class Parser9 {
    ...
    public void parseOutExcel2(Map<String,String> cellData){
        ...
        for (Map.Entry entry : cellData.entrySet()){
            ...
            System.out.println("Номер строки: " + cellReference.getRow());
            row = null;
            ...
            //while (cells.hasNext()){
                ...
                System.out.println("Номер колонки: " + cellReference.getCol()+ "Значение: " + entry.getValue());
                cell.setCellValue(String.valueOf(entry.getValue()));
            //}
            utils.saveWorkbook(outputWorkbook, outputFile);
        }

        /*Iterator<Row> it = outputFileSheet.iterator();
        while (it.hasNext()){
            Row row = it.next();
            Iterator<Cell> cells = row.iterator();
            while (cells.hasNext()){
        ...
        /*for (Map.Entry entry : cellData.entrySet()){
          ...
          //  while (it.hasNext()){
         //       Row row = it.next();
            Row row = outputFileSheet.createRow(cellReference.getRow());
            System.out.println("Номер строки: " + row.getRowNum());

            //Iterator<Cell> cells = row.iterator();
            //while (cells.hasNext()){
                //Cell cell = cells.next();
                cell = row.createCell(cellReference.getCol());
                System.out.println("Номер столбца: " + cell.getColumnIndex());
                cell.setCellValue(String.valueOf(entry.getValue()));
           // }
       // }
           // utils.saveWorkbook(outputWorkbook, outputFile);
